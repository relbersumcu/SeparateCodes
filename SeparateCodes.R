...
Separate_codes <- function(codesheet,code_col,separator,vars){
  new2 <-list()
    ...
    new <-list()
        ...
        new[[i]] <- cbind(codesheet1[i,][rep(seq_len(nrow(codesheet1[i,])), each = length(strsplit(codesheet1[i,j],separator)[[1]])),], "Code_new" =  strsplit(codesheet1[i,j], ',')[[1]])
